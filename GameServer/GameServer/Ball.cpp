...
{
	std::shared_lock<std::shared_mutex> sharedLock(_ballMutex);
	...
	std::chrono::milliseconds preSyncDur(user->GetLastSyncTime());
	std::chrono::system_clock::time_point preSyncTimePoint(preSyncDur);
	float syncElapsed = std::chrono::duration<float>(_hitTime - preSyncTimePoint).count();
...
BallResult Ball::Sync(SysTp syncReqTime)
{
	std::shared_lock<std::shared_mutex> sharedLock(_ballMutex);
	if (_lastHitterIdx < 0)
	{
	...
	std::chrono::duration<float> syncElapsed = duration_cast<std::chrono::duration<float>>(syncReqTime - _hitTime);
